...
while(True):
   ...
   desiredPosition[0] = q1_traj.getPostionTrajectory(currentTime=t)
   desiredPosition[1] = q2_traj.getPostionTrajectory(currentTime=t)
   desiredVelocity[0] = q1_traj.getVelocityTrajectory(currentTime=t)
   desiredVelocity[1] = q2_traj.getVelocityTrajectory(currentTime=t)
   desiredAcc[0] = q1_traj.getAccelerationTrajectory(currentTime=t)
   desiredAcc[1] = q2_traj.getAccelerationTrajectory(currentTime=t)
   ...
   #qqq = desiredAcc + 100.0*(desiredPosition-q) + 10.0*(desiredVelocity-dq)
   temptorque = np.dot(massMat,desiredAcc + 100.0*(desiredPosition-q) + 10.0*(desiredVelocity-dq)) #+ np.dot(coriolisMat,desiredVelocity)
